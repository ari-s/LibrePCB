                                     ...
                                     BI_NetPoint* fixedPoint) noexcept
{
    try
    {
        ...
        } else if (BI_NetPoint* netpoint = findNetPoint(board, pos, *layer)) {
        ...
        } else if (BI_FootprintPad* pad = findPad(board, pos, *layer)) {
            mFixedStartAnchor = pad;
            netsegment = pad->getNetSegmentOfLines();
            netsignal = pad->getCompSigInstNetSignal();
        } else if (BI_NetLine* netline = findNetLine(board, pos, *layer)) {
            // split netline
...
BI_NetPoint* BES_DrawTrace::findNetPoint(Board& board, const Point& pos,
                                         ...
                                         BI_NetPoint* except) const noexcept
{
    QList<BI_NetPoint*> items = board.getNetPointsAtScenePos(pos, &layer, netsignal);
    items.removeAll(except);
    return items.count() > 0 ? items.first() : nullptr;
}

BI_NetLine* BES_DrawTrace::findNetLine(Board& board, const Point& pos, GraphicsLayer& layer,
                                       NetSignal* netsignal, BI_NetLine* except) const noexcept
{
    QList<BI_NetLine*> items = board.getNetLinesAtScenePos(pos, &layer, netsignal);
    items.removeAll(except);
    return items.count() > 0 ? items.first() : nullptr;
}
